...
class Env {
 public:
  ...
  using RandomAccessFile = string;
  using WritableFile = string;
  using ReadOnlyMemoryRegion = int;
  ...
  ///                    implemented
  Status DeleteRecursively(const string& dirname, int* undeleted_files,
                           int* undeleted_dirs);
  ...
  Status Stat(const string& fname, string* stat);
  ...
  Status GetFileSize(const string& fname, uint* file_size);
  ...
  virtual uint NowMicros() { return 0; };

  /// \brief Returns the number of seconds since the Unix epoch.
  virtual uint NowSeconds() { return 0; }
  ...
  virtual void SchedClosureAfter(int micros,
 ...
 private:
  ...
  std::unique_ptr<string> file_system_registry_;
...
class EnvWrapper : public Env {
 public:
  ...
  uint NowMicros() override { return target_->NowMicros(); }
